...
- **Procesar** — What Librarian does with a Mensaje: classify the intent, execute, return a result. For `/process`, this includes automatic approval and application of proposals to the wiki.
...
- **Proposal** — A suggested change to the vault (create, update, or skip). Stored in `.librarian/proposals/`. During `/process`, proposals are auto-approved and applied.
- **Review** — The approval/rejection of a Proposal by the user. Available for manual review via `/review` overlay or CLI commands.
