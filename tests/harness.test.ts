...
test("harness routes search queries to semantic search", async () => {
  const ctx = await createTestContext({
    "wiki/conceptos/clean-architecture.md": "Clean Architecture protects business rules and architecture decisions.\n",
  });

  const result = await runLibrarian("buscar Clean Architecture", ctx.vaultPath);
...
test("harness reports wiki status with stats and graph", async () => {
  const ctx = await createTestContext({
    "wiki/conceptos/clean-architecture.md": ["---", "librarian:", "  status: active", "---", "", "Clean Architecture."].join("\n"),
  });

  const result = await runLibrarian("estado de la wiki", ctx.vaultPath);
